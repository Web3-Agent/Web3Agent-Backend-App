...
export const login = async (request: Request, response: Response) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id }, ENV_VARIABLES.JWT_SECRET, {
...
export const logout = async (request: CustomRequest, response: Response) => {
  try {
    ...
    const isValidToken = jwt.verify(token, ENV_VARIABLES.JWT_SECRET);
    if (!isValidToken) {
      return response.status(401).json({
        success: false,
...
export const validateToken = async (request: Request, response: Response) => {
  try {
    ...
    const isValidToken: any = jwt.verify(token, ENV_VARIABLES.JWT_SECRET);
